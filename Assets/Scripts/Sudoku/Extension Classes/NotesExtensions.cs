    ...
    public static class NotesExtensions
    {
        ...
        public static void Update(this bool[,,] notes, int[,] sudoku, (int row, int col) cellIndex, int removedValue, int placedValue)
        {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                if (!isZero) notes[cellIndex.row, cellIndex.col, i] = false;
                ...
                notes[row, col, noteIndex] = isZero && sudoku.CanUseNumber(row, col, noteIndex + 1);

                // Row cells, only the ones outside of the box.
                if (i < boxCoords.row || i > boxCoords.row + 2) {
                    notes[i, cellIndex.col, noteIndex] = isZero && sudoku.CanUseNumber(i, cellIndex.col, noteIndex + 1);
                }

                // Column cells, only the ones outside of the box.
                if (i < boxCoords.col || i > boxCoords.col + 2) {
                    notes[cellIndex.row, i, noteIndex] = isZero && sudoku.CanUseNumber(cellIndex.row, i, noteIndex + 1);
                }
            }
        }
        ...
        public static void SetNotes(this bool[,,] notes, int[,] puzzle)
        {
            for (int row = 0; row < 9; row++)
                for (int col = 0; col < 9; col++) {
                    ...
                    for (int i = 0; i < 9; i++)
                        notes[row, col, i] = isEmpty && puzzle.CanUseNumber(row, col, i + 1);
                }
        }
        ...
        public static void UpdateOnCellEdit(this bool[,,] notes, (int row, int col) cellIndex, int newValue)
        {
            ...
            for (int i = 0; i < 9; i++) {
                // Disable all notes in this cell if the new value is not zero.
                notes[cellIndex.row, cellIndex.col, i] = false;
                ...
                notes[i, cellIndex.col, noteIndex] = notes[cellIndex.row, i, noteIndex] = notes[row, col, noteIndex] = false;
            }
        }
    }
}